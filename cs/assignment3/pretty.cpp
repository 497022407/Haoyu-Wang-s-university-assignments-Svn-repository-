...
void walk_var_dec(ast t, bool is_para) ;
...
void walk_class_var_decs(ast t)
{
    ...
    for ( int i = 0 ; i < ndecs ; i++ )
    {
        ...
        if (get_var_dec_segment(temp) == "static")
        {
            end_line = true;
            print_tab();
    ...
    {
        ast temp = get_class_var_decs(t,i);
        if (get_var_dec_segment(temp) == "this")
        {
            end_line = true;
            print_tab();
            walk_var_dec(get_class_var_decs(t,i), false) ;
...
//
void walk_var_dec(ast t, bool is_para = false)
{
...
void walk_unary_op(ast t)
{
    ...
    if (uop == "~" && ast_node_kind(get_term_term(term)) == ast_expr)
    {
        cout << "(" ;
    }
    walk_term(term) ;
