...
ast copy_var_dec(ast t, bool is_dec) ;
...
std::vector<ast> used_variables;

bool is_used(ast variable)
{
    string name = get_var_name(variable);
    string segment = get_var_segment(variable);
    int count = used_variables.size();
    for (int i = 0; i < count; ++i)
    {
        ast var = used_variables[i];
        if (get_var_name(var) == name)
        {
            if (get_var_segment(var) == segment)
            {
                return true;
            }
        }
...
ast copy_class_var_decs(ast t)
{
    ...
    for ( int i = 0 ; i < ndecs ; i++ )
    {
        ...
        ast copy = copy_var_dec(deci, true) ;
        if ( deci != copy ) copied = true ;

        if (ast_node_kind(copy) != ast_empty)
        {
            decs.push_back(copy) ;
        }
    }
...
//
ast copy_var_dec(ast t, bool is_dec = false)
{
    ...
    if (is_dec == true)
    {
        ast var = create_var(name, segment, offset, type) ;
        if (is_used(var) || segment == "this")
        {
             return t ;
        }
        else
...
ast copy_statements(ast t)
{
    ...
    for ( int i = 0 ; i < size ; i++ )
    {
        ...
        ast cpoy_s = get_statement_statement(copy) ;
        if ( deci != copy ) copied = true ;
        
        if (ast_node_kind(cpoy_s) != ast_empty)
        {
            decs.push_back(copy) ;
        }
        if (ast_node_kind(cpoy_s) == ast_return ||ast_node_kind(cpoy_s) == ast_return_expr)
        {
...
ast copy_if(ast t)
{
    ...
    if (ast_node_kind(get_term_term(get_expr(condition_copy, 0))) == ast_bool)
    if (size_of_expr(condition_copy) == 1 && !get_bool_t_or_f(get_term_term(get_expr(condition_copy, 0))))
    {
        return create_empty();
    }
    if (size_of_statements(if_true_copy) == 0)
    {
